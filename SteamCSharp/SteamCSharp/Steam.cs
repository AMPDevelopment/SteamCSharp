...
using SteamCSharp.Handler.Games;
using SteamCSharp.Models.Community;
using SteamCSharp.Models.Community.Statistics;
using SteamCSharp.Models.Community.Statistics.Game;
using SteamCSharp.Models.Community.Statistics.User;
...
namespace SteamCSharp
{
    public sealed class Steam
    {
        ...
        /// <param name="steamId">Steam ID 64</param>
        /// <returns></returns>
        public async Task<SteamUserStatsForGameResult> GetSteamUserStatsForGameAsync(uint appId, ulong steamId)
        {
            ...
            return JsonConvert.DeserializeObject<SteamUserStatsForGameResponse>(request).Result;
        }
        ...
        /// <param name="steamId">Steam ID 64</param>
        /// <returns></returns>
        public async Task<GlobalOffensive> GetSteamUserStatsForGlobalOffensiveAsync(ulong steamId)
        {
            var parameters = $"{AppIdParameter}730{SteamIdParameter}{steamId}";
            var request = await GetAsync(SteamUserStatsUrl, GetUserStatsForGameUrl, Version2, parameters);
            var stats = JsonConvert.DeserializeObject<SteamUserStatsForGameResponse>(request).Result.Stats;

            return CounterStrike.ReadableStats(stats);
        }
